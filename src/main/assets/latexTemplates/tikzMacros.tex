...
\usepackage[paperheight=297mm,paperwidth=210mm,margin=1cm,heightrounded,footskip=0.5cm]{geometry}
...
\tikzset {  % TODO: Add style options that distinguish nodes/edges that are from rule 1 / rule 2 or both
    ...
    handle node/.style={fill,circle,inner sep=2pt,electric charge=2},
    ...
    global heap drawing settings/.style={spring electrical layout,node distance=2cm,electric force order=3},
    ...
    \ifx\isExternal\true
        ...
        \else
            ...
            \ifx\currentHeapPath\pathRHS
                ...
                \node[external node] (node#2) {$(\externalIdices)$}; % TODO
            \else
                % Is normal external node
                \node[external node] (node#2) {$#2(\externalIdices)$};
            \fi
        \fi
    \else
        \ifx\currentHeapPath\pathJointGraph
            % Is node in joint graph of a critical pair
            \node[internal node] (node#2) {#2};
