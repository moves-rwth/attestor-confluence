...
\tikzset {  % TODO: Add style options that distinguish nodes/edges that are from rule 1 / rule 2 or both
    ...
    hyperedge node/.style={draw,rectangle,electric charge=1},
    ...
    global heap drawing settings/.style={spring electrical layout,node distance=1.5cm,electric force order=3},
...
\newcommand{\AttestorDrawNode}[2] {
    ...
    % Normal external node
    \node[external node] (node\id) {$\id(\external)$};
    % External node in handle
    \node[handle node] (n3) {};
    % Internal Node
